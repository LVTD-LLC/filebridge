...
For a self-hosted installation, Qdrant is an optional private Compose service. Set its generated
...
make manage backfill_dataset_vectors --all --stop-on-error
```
...
owner-only `.env` described in [SELF_HOSTING.md](SELF_HOSTING.md), then run:
...
The production stack mounts the named `media_data` and `private_media_data` volumes into both the
...
Follow [SELF_HOSTING.md](SELF_HOSTING.md) for the complete setup, verification, backup, update, and
