...
action_refs="$(
	rg --no-filename -o 'uses:[[:space:]]+[^[:space:]#]+' \
...
for runner in ubuntu-latest ubuntu-24.04-arm macos-15-intel macos-15; do
	if ! rg -q "runner: $runner" "$publish_workflow"; then
		printf 'missing native CLI release runner: %s\n' "$runner" >&2
		exit 1
	fi
done

if ! rg -q 'name: Smoke-test CLI archive' "$publish_workflow"; then
	printf 'release workflow does not smoke-test final CLI archives\n' >&2
	exit 1
fi

if ! rg -q 'permissions:[[:space:]]*$' "$publish_workflow" ||
	! rg -q 'contents: read' "$publish_workflow"; then
